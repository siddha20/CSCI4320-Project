...
well as the much larger voting data that results from these additions, we had
to cut down the number of votes tested by an order of magnitude to ensure that
we could get results on AiMOS in a reasonable amount of time. The results shown
in Figure 7 show the total time needed to encrypt and hash the generated voting
...
\begin{description}
    ...
    \item[Alex Riddle] Implemented AES-128, AES-192, AES-256, SHA-1, AES-CTR,
    multi-threaded AES, and multi-threaded SHA-1.
\end{description}
