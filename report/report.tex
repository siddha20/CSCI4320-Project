...
well as the much larger voting data that results from these additions (each
SHA-1 digest is 20 bytes, and we have many hashes for each chunk of the file),
we had to cut down the number of votes tested by an order of magnitude to ensure
that we could get results on AiMOS in a reasonable amount of time. The results
shown in Figure 7 show the total time needed to encrypt and hash the generated
voting data file. Similar to previous results, we can see that there is little
benefit to using more ranks with vote counts around 10,000. Though, in this
case, we can actually observe a minor speedup right at 10,000 votes: 256 ranks
versus 32 ranks shows that 256 ranks has a slight edge in speed, though it is so
slight that this number of ranks would not be worth it for vote counts at or
around 10,000.
...
We see very similar running times for the encryption and hashing for vote counts
less than or around 10,000. Then, between 10,000 and 20,000 votes, we start to
see a clear divergence in running times between the numbers of ranks. The
general trend is that as the number of ranks increases, the running times
decrease, which becomes more apparent as the number of votes (i.e., the total
size of the file) increases. The one strange exception is 32 ranks: for some
reason, 32 ranks is consistently slower than 8 ranks, despite 32 ranks clearly
providing more computational capacity. This same phenomenon can be observed in
the above figures as well, particularly Figure 4.
...
% \begin{center}
%     \includegraphics[scale=.6]{../aimos-stats/collection9/plots/decrypt_time.png}
%     \captionof{figure}{Compares the total time versus the number of votes for
%              generating and encrypting candidate listings. Each rank has its
%              own line indicated by the legend. Each point represents an actual
%              data point (access
